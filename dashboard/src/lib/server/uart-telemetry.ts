...
function parseBeaconPayload(
	...
	if (columns.length < 16 || columns[0] !== "beacon_nav") {
		return null;
	}
...
class UartTelemetryService {
	...
	private rawLineListeners = new Set<RawLineListener>();
	private statusListeners = new Set<StatusListener>();
	private frames: TelemetryFrame[] = [];
	private rawLines: UartRawLine[] = [];
	...
	getRecentRawLines(limit = 120): UartRawLine[] {
		return this.rawLines.slice(-limit);
	}
	...
	private emitRawLine(raw: UartRawLine): void {
	...
	private async resolvePortPath(): Promise<string | null> {
		...
		try {
			...
			const preferred = ports.find((port) =>
				PREFERRED_PORT_PATTERN.test(port.path),
			);
	...
	private handleLine(rawLine: string): void {
		...
		this.status.lastLineAtMs = receivedAtMs;
		const frame = parseTelemetryLine(line);
		this.status.rawLinesReceived++;

		const raw: UartRawLine = {
			receivedAtMs,
			line,
			parsed: frame !== null,
		};
		this.rawLines.push(raw);
		if (this.rawLines.length > MAX_RAW_BUFFER) {
			this.rawLines.splice(0, this.rawLines.length - MAX_RAW_BUFFER);
		}
		this.emitRawLine(raw);
...
const uartTelemetry =
	globalState.__uartTelemetryService ?? new UartTelemetryService();
